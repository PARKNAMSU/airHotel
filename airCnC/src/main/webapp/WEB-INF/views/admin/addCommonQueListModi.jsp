...
<head>
	...
	<script>
		...
		function UpdateSubmit() {
			...
			f.action = '<c:url value="/updateCommonQue.mdo${paging.makeQueryPage(detail.commonq_idx, page) }" />';
			f.submit();
		}
		function DeleteSubmit() {
			var f = document.form1;
			f.action = "<c:url value='/deleteCommonQue.mdo${paging.makeQueryPage(detail.commonq_idx, page) }' />";
			f.submit();
		}
		function ListSubmit() {
			var f = document.form1;
			f.action = '<c:url value="/goCommonQueListAdmin.mdo${paging.makeQueryPage(page) }" />';
			f.submit();
		}
...
<header class="logo" style="position: sticky;">
     <label for="" class="loglabel">자주묻는질문 수정(관리자)</label>
      <span>
	...
	<div class="title">
					...
					<hr>
					<span class="byline">자주묻는질문을 수정합니다.</span><br><br>
				</div>
		<ul>
            <li>
                ...
                <label>질문의 유형을 선택해주세요.</label><br /><br />
					...
					<br></br>
          				...
          				onblur="if(this.value == '') { this.value = '새로운 질문의 내용을 입력하세요.'; }">${detail.commonq_content }</textarea>

          				<input type="hidden" name="commonq_idx" value="${detail.commonq_idx }">
