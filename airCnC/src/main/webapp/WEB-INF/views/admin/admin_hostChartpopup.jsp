...
<body>
	<div style="margin-left: 50px;">
		...
		<form action="" >
			<input type="date" name="hostchart_date" class="form-control"><br>
			<input type="submit" value="검색" class="btn btn-outline-danger"> 
		</form><br>
		</div>
		<div id="salesArea">
			<h1>${hostList.get(0).host_image } 신청</h1>
			<table class="table table-striped" id="salesTable">
				...
				<c:if test="${hostList != null}">
					...
					<tr>
						<td>${hostList.get(i).host_image }</td>
						<td>${hostList.get(i).host_id }</td>
						<td>${hostList.get(i).host_name }</td>
						<td>${hostList.get(i).host_email }</td>
						<td>${hostList.get(i).host_phone }</td>
						<td>${hostList.get(i).host_account }</td>
						<td>${hostList.get(i).host_blame_warn }</td>
						<td>${hostList.get(i).host_blame_stop }</td>
					</tr>
					</c:forEach>
				</c:if>
			</table>
