...
<head>
	...
	<script>
		...
		function UpdateSubmit() {
			...
			f.action = '<c:url value="/updatePolicy.mdo${paging.makeQueryPage(detail.policy_idx, page) }" />';
			f.submit();
		}
		function DeleteSubmit() {
			var f = document.form1;
			f.action = "<c:url value='/deletePolicy.mdo${paging.makeQueryPage(detail.policy_idx, page) }' />";
			f.submit();
		}
		function ListSubmit() {
			var f = document.form1;
			f.action = '<c:url value="/goPolicyListAdmin.mdo${paging.makeQueryPage(page) }" />';
			f.submit();
		}
...
<body>
	<header class="logo" style="position: sticky;">
     <label for="" class="loglabel">약관정책 수정(관리자)</label>
      <span>
	...
	<div class="title">
					<h2 style="color: black;">약관정책 관리</h2>
					<hr>
					<span class="byline">약관정책을 수정합니다.</span><br><br>
				</div>
		<ul>
            <li>
                <form id="form1" name="form1" method="POST">

                   			<input name="policy_title" class="" type="text" value="${detail.policy_title }"
          					...
          					onblur="if(this.value == '') { this.value = '새로운 공지의 내용을 입력하세요.'; }">${detail.policy_content }</textarea>

							<input type="hidden" name="policy_idx" value="${detail.policy_idx }">
          					<a href="#" onclick="UpdateSubmit();" id="updateBtn" ><button>수정이라구</button></a>
          					<a href="#" onclick="DeleteSubmit();" id="deleteBtn" ><button>삭제라구</button></a>
          					<a href="#" onclick="ListSubmit();" id="listBtn" ><button>뒤로라구</button></a>
