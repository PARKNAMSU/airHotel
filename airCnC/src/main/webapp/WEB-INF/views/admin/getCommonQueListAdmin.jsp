...
<body>
     ...
     <label for="" class="loglabel">자주묻는질문(관리자)</label>
      <span>
	...
	<div id="featured">
			<div class="title">
				...
				<hr>
				<span class="byline">전체 자주묻는질문을 관리합니다.</span><br><br>
				<button id="gonjibtn" type="button" onclick="location.href='/cnc/goaddCommonQue.mdo' ">
				새 질문 등록</button>
			</div>
	...
	<!-- Start Pagination -->
		<ul class="pagination">
		    ...
		    <li>
		        <a href='<c:url value="/goCommonQueListAdmin.mdo${paging.makeQueryPage(paging.startPage-1) }"/>'>이전</a>
		    </li>
		    </c:if>
		    <c:forEach begin="${paging.startPage }" end="${paging.endPage }" var="pageNum">
		    <li>
		        <a href='<c:url value="/goCommonQueListAdmin.mdo${paging.makeQueryPage(pageNum) }"/>'><i class="fa">${pageNum }</i></a>
		    </li>
		    </c:forEach>
		    <c:if test="${paging.next && paging.endPage >0 }">
		    <li>
		        <a href='<c:url value="/goCommonQueListAdmin.mdo${paging.makeQueryPage(paging.endPage+1) }"/>'>다음</a>
		    </li>
		    </c:if>
		</ul>
	<!-- End Pagination -->
