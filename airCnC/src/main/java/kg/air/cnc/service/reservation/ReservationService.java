...
public class ReservationService implements ReservationServiceImpl {
	...
	public ReservationHouseDetailVO getReservationHouse(ReservationHouseDetailVO vo) {
		...
		if(house.getHouse_default_bedrock().trim().equals("true")) convinList.add("침실잠금");
		
		if(house.getHouse_default_coditecter().trim().equals("true")) convinList.add("일산화탄소 감지기");
		
		if(house.getHouse_default_cooler().trim().equals("true")) convinList.add("냉방장치");
		
		if(house.getHouse_default_fireditecter().trim().equals("true")) convinList.add("화재감지기");
		...
		if(house.getHouse_defaultsetting().trim().equals("true")) convinList.add("필수품목");
		...
		if(house.getHouse_condition_partyok().trim().equals("true"))conditionList.add("파티 허용");
		if(house.getHouse_condition_smokeok().trim().equals("true"))conditionList.add("흡연 가능");
		...
		if(house.getHouse_restrict_beast().trim().equals("true"))restrictList.add("위험 동물 주의");
		if(house.getHouse_restrict_cantpark().trim().equals("true"))restrictList.add("건물 내 주차 금지");
		if(house.getHouse_restrict_cctv().trim().equals("true"))restrictList.add("CCTV 있음");
		if(house.getHouse_restrict_commonspace().trim().equals("true"))restrictList.add("일부 공용공간 있음");
		if(house.getHouse_restrict_facility().trim().equals("true"))restrictList.add("편의시설 제한");
		if(house.getHouse_restrict_pet().trim().equals("true"))restrictList.add("숙소에 반려동물 있음");
		if(house.getHouse_restrict_noise().trim().equals("true"))restrictList.add("소음 발생할 수 있음");
		if(house.getHouse_restrict_stairs().trim().equals("true"))restrictList.add("계단 이용해야함");
		if(house.getHouse_restrict_weapon().trim().equals("true"))restrictList.add("무기 있음");
