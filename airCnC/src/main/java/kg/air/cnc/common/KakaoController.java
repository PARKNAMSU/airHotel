...
public class KakaoController {
	...
	private String getKakaoRedirectURI(HttpServletRequest httpServletRequest){
		String uri =  httpServletRequest.getScheme()+"://"+httpServletRequest.getServerName()+":"+httpServletRequest.getServerPort();
		uri = uri+"/cnc/kakaologin.do";
		return uri;
	}
	
	public  String getAuthorizationUrl(HttpSession session, HttpServletRequest httpServletRequest) {
		String kakaoUrl = "https://kauth.kakao.com/oauth/authorize?" + "client_id=" + kakaoClientId + "&redirect_uri=" + getKakaoRedirectURI(httpServletRequest) + "&response_type=code";
		return kakaoUrl;
	}

	// 카카오 로그인.
	public JsonNode getAccessToken(String autorize_code, HttpServletRequest httpServletRequest) {
		final String RequestUrl = "https://kauth.kakao.com/oauth/token";
		final List<NameValuePair> postParams = new ArrayList<NameValuePair>();
		postParams.add(new BasicNameValuePair("grant_type", "authorization_code"));
		postParams.add(new BasicNameValuePair("client_id", kakaoClientId)); // REST API KEY.
		postParams.add(new BasicNameValuePair("redirect_uri", getKakaoRedirectURI(httpServletRequest))); // 리다이렉트 URI.
